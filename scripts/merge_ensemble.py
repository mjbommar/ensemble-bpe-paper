...
from ebpe.ensemble import build_merge_voted_bpe_json, build_merge_weighted_bpe_json
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--weight-by", choices=["none", "bytes", "lines", "entropy", "uniq_bytes", "chars", "uniq_chars", "char_entropy", "custom"], default="none",
                    help="how to weight members when voting (requires shard_stats in ensemble.json unless 'custom')")
    ...
    if args.ensemble_run:
        ...
        # Try to read shard_stats from ensemble.json for weighting
        try:
            j = json.loads((ens_dir / "ensemble.json").read_text(encoding="utf-8"))
            ens_stats = [m.get("shard_stats", {}) for m in j.get("members", [])]
        except Exception:
            ens_stats = None
    elif args.members:
    ...
    meta = {"members": [str(p) for p in members]}
    ...
    if k_used is not None:
        meta["k"] = k_used
    write_text(paths["env"], f"merge_ensemble members={len(members)} k={k_used} theta={theta} weight_by={args.weight_by}\n")

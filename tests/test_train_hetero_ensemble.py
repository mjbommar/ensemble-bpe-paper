...
def test_train_hetero_ensemble_selects_best(tmp_path: Path):
    ...
    out = subprocess.check_output(
        [
            ...
            "python", "-m", "scripts.train_hetero_ensemble",
            "--exp", "het_ens_pytest",
            "--train", str(train),
            "--eval-file", str(eval_file),
            "--algos", "hf_bpe", "hf_wordpiece", "hf_unigram",
            "--vocab-size", "64",
    ...
    assert (ens_dir / "hetero_ensemble.json").exists()
    assert (ens_dir / "selected_tokenizer.json").exists()
    sel = json.loads((ens_dir / "hetero_ensemble.json").read_text(encoding="utf-8"))
